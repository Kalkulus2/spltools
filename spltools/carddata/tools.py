...
def in_set(card_id, set_, card_data):
    ...
    set_ : str or int
        String or integer id for the set (case insensitive). Valid names are 
        "alpha", "beta", "untamed", "gladius", "chaos", "rebellion", and valid
        integers are 0, 1, 4, 6, 7, and 12.
    ...
    if(isinstance(set_, str)):
        if(set_.lower() in settings.set_str_to_int.keys()):
            ...
            raise ValueError(f"Valid set strings are \
                {list(settings.set_str_to_int.keys())}")
    else:
        if(not set_ in settings.set_str_to_int.values()):
            raise ValueError(f"Valid set values are \
                {list(settings.set_str_to_int.values())}")
    ...
    if set_ == card_edition:
        ...
        if(set_ == 0):
            ...
            if(card_edition == 2 and tier is None and card_id > 78):# Beta promo  
                return True
